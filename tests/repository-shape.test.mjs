...
const skills = ['audit','audit-repair-loop','art-production','fix','verification','release','visual-qa','prod-check','incident-analysis'];
...
test('AGENTS.md remains a compact router', () => {
  ...
  assert.match(text, /art-production/i);
});
...
test('global Codex layer and custom model-routing agents exist', () => {
  ...
  assert.match(globalInstructions, /ceos_art_director_web/i);
  assert.match(globalInstructions, /ceos_asset_generator/i);
